...
public abstract class GenericUtility(RotationModuleManager manager, Actor player) : RotationModule(manager, player)
{
    ...
    protected static RotationModuleDefinition.ConfigRef<LBOption> DefineLimitBreak<Index>(RotationModuleDefinition def, Index expectedIndex, ActionTargets allowedTargets, float effectLB1 = 0, float effectLB2 = 0, float effectLB3 = 0) where Index : Enum
    {
        ...
        return def.Define(expectedIndex).As<LBOption>("LB")
            ...
            .AddOption(LBOption.LB3, "Use LB3 if available", cooldown: 0, effect: effectLB3, supportedTargets: allowedTargets, defaultPriority: ActionQueue.Priority.VeryHigh)
            .AddOption(LBOption.LB2, "Use LB2/3 if available", cooldown: 0, effect: effectLB3, supportedTargets: allowedTargets, defaultPriority: ActionQueue.Priority.VeryHigh)
            .AddOption(LBOption.LB1, "Use any LB if available", cooldown: 0, effect: effectLB3, supportedTargets: allowedTargets, defaultPriority: ActionQueue.Priority.VeryHigh)
            .AddOption(LBOption.LB2Only, "Use LB2 if available, but not LB3", cooldown: 0, effect: effectLB2, supportedTargets: allowedTargets, defaultPriority: ActionQueue.Priority.VeryHigh)
            .AddOption(LBOption.LB1Only, "Use LB1 if available, but not LB2+", cooldown: 0, effect: effectLB1, supportedTargets: allowedTargets, defaultPriority: ActionQueue.Priority.VeryHigh)
            .AddOption(LBOption.LB12, "Use LB1/2 if available, but not LB3", cooldown: 0, effect: effectLB1, supportedTargets: allowedTargets, defaultPriority: ActionQueue.Priority.VeryHigh);
    }
