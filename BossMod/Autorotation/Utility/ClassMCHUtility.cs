...
public sealed class ClassMCHUtility(RotationModuleManager manager, Actor player) : RoleRangedUtility(manager, player)
{
    ...
    public static RotationModuleDefinition Definition()
    {
        ...
        res.Define(Track.Tactician).As<TactOption>("Tactician", "Tact", 400)
            ...
            .AddOption(TactOption.Use87, "Use Tactician (120s CD), regardless if equivalent ranged buff is already active", cooldown: 120, effect: 15, supportedTargets: ActionTargets.Self, minLevel: 56, maxLevel: 87)
            .AddOption(TactOption.Use87IfNotActive, "Use Tactician (120s CD), unless equivalent ranged buff is already active", cooldown: 90, effect: 15, supportedTargets: ActionTargets.Self, minLevel: 56, maxLevel: 87)
            .AddOption(TactOption.Use88, "Use Tactician (90s CD), regardless if equivalent ranged buff is already active", cooldown: 90, effect: 15, supportedTargets: ActionTargets.Self, minLevel: 88)
            .AddOption(TactOption.Use88IfNotActive, "Use Tactician (90s CD), unless equivalent ranged buff is already active", cooldown: 90, effect: 15, supportedTargets: ActionTargets.Self, minLevel: 88)
