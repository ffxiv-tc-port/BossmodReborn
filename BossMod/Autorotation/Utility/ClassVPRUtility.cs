...
public sealed class ClassVPRUtility(RotationModuleManager manager, Actor player) : RoleMeleeUtility(manager, player)
{
    ...
    public static RotationModuleDefinition Definition()
    {
        ...
        res.Define(Track.Slither).As<DashStrategy>("Slither", "", 20)
            ...
            .AddOption(DashStrategy.GapClose, "Use as gapcloser if outside melee range; uses all charges if needed", cooldown: 30, effect: 0, supportedTargets: ActionTargets.Party | ActionTargets.Hostile, minLevel: 35)
            .AddOption(DashStrategy.GapCloseHold1, "Use as gapcloser if outside melee range; holds 1 charge for manual usage", cooldown: 30, effect: 0, supportedTargets: ActionTargets.Party | ActionTargets.Hostile, minLevel: 35)
            .AddOption(DashStrategy.GapCloseHold2, "Use as gapcloser if outside melee range; holds 2 charges for manual usage", cooldown: 30, effect: 0, supportedTargets: ActionTargets.Party | ActionTargets.Hostile, minLevel: 84)
