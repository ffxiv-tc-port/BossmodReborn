...
public sealed class ClassBRDUtility(RotationModuleManager manager, Actor player) : RoleRangedUtility(manager, player)
{
    ...
    public static RotationModuleDefinition Definition()
    {
        ...
        res.Define(Track.Troubadour).As<TroubOption>("Troubadour", "Troub", 500)
            ...
            .AddOption(TroubOption.Use87, "Use Troubadour (120s CD), regardless if equivalent ranged buff is already active", cooldown: 120, effect: 15, supportedTargets: ActionTargets.Self, minLevel: 62, maxLevel: 87)
            .AddOption(TroubOption.Use87IfNotActive, "Use Troubadour (120s CD), unless equivalent ranged buff is already active", cooldown: 90, effect: 15, supportedTargets: ActionTargets.Self, minLevel: 62, maxLevel: 87)
            .AddOption(TroubOption.Use88, "Use Troubadour (90s CD), regardless if equivalent ranged buff is already active", cooldown: 90, effect: 15, supportedTargets: ActionTargets.Self, minLevel: 88)
            .AddOption(TroubOption.Use88IfNotActive, "Use Troubadour (90s CD), unless equivalent ranged buff is already active", cooldown: 90, effect: 15, supportedTargets: ActionTargets.Self, minLevel: 88)
