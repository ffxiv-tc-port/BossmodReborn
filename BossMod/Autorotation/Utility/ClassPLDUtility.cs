...
public sealed class ClassPLDUtility(RotationModuleManager manager, Actor player) : RoleTankUtility(manager, player)
{
    ...
    public static RotationModuleDefinition Definition()
    {
        ...
        res.Define(Track.Sheltron).As<ShelOption>("Sheltron", "Shel", 350) //Sheltron definitions
            ...
            .AddOption(ShelOption.Sheltron, "Use Sheltron", cooldown: 0, effect: 4, supportedTargets: ActionTargets.Self, minLevel: 35, maxLevel: 81) //5s CD, 6s duration, -50 OathGauge cost
            .AddOption(ShelOption.HolySheltron, "Use Holy Sheltron", cooldown: 0, effect: 4, supportedTargets: ActionTargets.Self, minLevel: 82) //5s CD, 8s duration (similar to GNB/WAR we only really care about the first 4s of the mit), -50 OathGauge cost
            .AddOption(ShelOption.Intervention, "Use Intervention", cooldown: 0, effect: 4, supportedTargets: ActionTargets.Party, minLevel: 62) //10s CD, 8s duration (similar to GNB/WAR we only really care about the first 4s of the buddy mit), -50 OathGauge cost
        ...
        res.Define(Track.Sentinel).As<SentOption>("Sentinel", "Sent", 550) //Sentinel definition for CD plans
            ...
            .AddOption(SentOption.Sentinel, "Use Sentinel", cooldown: 120, effect: 15, supportedTargets: ActionTargets.Self, minLevel: 38, maxLevel: 91) //120s CD, 15s duration
            .AddOption(SentOption.Guardian, "Use Guardian", cooldown: 120, effect: 15, supportedTargets: ActionTargets.Self, minLevel: 92) //120s CD, 15s duration
        ...
        res.Define(Track.PassageOfArms).As<ArmsDirection>("PassageOfArms", "PoA", 400) //PassageOfArms definition for CD plans
            ...
            .AddOption(ArmsDirection.CharacterForward, "Faces the Forward direction relative to the Character", cooldown: 120, effect: 18, supportedTargets: ActionTargets.Self, minLevel: 70)
            .AddOption(ArmsDirection.CharacterBackward, "Faces the Backward direction relative to the Character", cooldown: 120, effect: 18, supportedTargets: ActionTargets.Self, minLevel: 70)
            .AddOption(ArmsDirection.CameraForward, "Faces the Forward direction relative to the Camera", cooldown: 120, effect: 18, supportedTargets: ActionTargets.Self, minLevel: 70)
            .AddOption(ArmsDirection.CameraBackward, "Faces the Backward direction relative to the Camera", cooldown: 120, effect: 18, supportedTargets: ActionTargets.Self, minLevel: 70)
