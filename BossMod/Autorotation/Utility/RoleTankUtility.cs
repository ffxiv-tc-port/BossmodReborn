...
public abstract class RoleTankUtility(RotationModuleManager manager, Actor player) : GenericUtility(manager, player)
{
    ...
    protected static void DefineShared(RotationModuleDefinition def, ActionID lb3, ActionID stanceApply, ActionID stanceRemove)
    {
        ...
        def.Define(SharedTrack.Reprisal).As<ReprisalOption>("Reprisal", "", 250)
            ...
            .AddOption(ReprisalOption.Use, "Use Reprisal (10s)", cooldown: 60, effect: 10, supportedTargets: ActionTargets.Self, minLevel: 22, maxLevel: 97)
            .AddOption(ReprisalOption.UseEx, "Use Reprisal (15s)", cooldown: 60, effect: 15, supportedTargets: ActionTargets.Self, minLevel: 98)
        ...
        def.Define(SharedTrack.Stance).As<StanceOption>("Stance", "", 5)
            ...
            .AddOption(StanceOption.Apply, "Use stance if not already active", cooldown: 2)
            .AddOption(StanceOption.Remove, "Remove stance if active", cooldown: 1)
