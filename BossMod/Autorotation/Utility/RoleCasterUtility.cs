...
public abstract class RoleCasterUtility(RotationModuleManager manager, Actor player) : GenericUtility(manager, player)
{
    ...
    protected static void DefineShared(RotationModuleDefinition def, ActionID lb3)
    {
        ...
        def.Define(SharedTrack.Addle).As<AddleOption>("Addle", "", 250)
            ...
            .AddOption(AddleOption.Use, "Use Addle (10s)", cooldown: 90, effect: 10, supportedTargets: ActionTargets.Hostile, minLevel: 22, maxLevel: 97)
            .AddOption(AddleOption.UseEx, "Use Addle (15s)", cooldown: 90, effect: 15, supportedTargets: ActionTargets.Hostile, minLevel: 98)
        ...
        def.Define(SharedTrack.Swiftcast).As<SwiftcastOption>("Swiftcast", "Swiftcast", 20)
            ...
            .AddOption(SwiftcastOption.Use, "Use Swiftcast (10s)", cooldown: 60, effect: 10, supportedTargets: ActionTargets.Self, minLevel: 22, maxLevel: 93)
            .AddOption(SwiftcastOption.UseEx, "Use Swiftcast (15s)", cooldown: 40, effect: 10, supportedTargets: ActionTargets.Self, minLevel: 94)
