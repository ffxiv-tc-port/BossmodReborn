...
public sealed class VeynWAR(RotationModuleManager manager, Actor player) : RotationModule(manager, player)
{
    ...
    public static RotationModuleDefinition Definition()
    {
        ...
        res.Define(Track.Potion).As<PotionStrategy>("Potion", uiPriority: 70)
            ...
            .AddOption(PotionStrategy.AlignWithRaidBuffs, "Align with 2-minute raid buffs (0/6, 2/8, etc)", cooldown: 270, effect: 30)
            .AddOption(PotionStrategy.AlignWithIR, "Align with IR, allow on odd minutes (0/5/10)")
            .AddOption(PotionStrategy.Immediate, "Use ASAP, even if without ST and with IR on cd (0/4:30/9)", cooldown: 270, effect: 30)
            .AddAssociatedAction(ActionDefinitions.IDPotionStr);

        res.Define(Track.PrimalRend).As<PrimalRendStrategy>("PR", uiPriority: 30)
            .AddOption(PrimalRendStrategy.Automatic, "Use normally: rend only when already in melee range, delay until raidbuffs if possible", cooldown: 0, effect: 0, supportedTargets: ActionTargets.Hostile, minLevel: 90)
            .AddOption(PrimalRendStrategy.Forbid, "Do not use automatically", cooldown: 0, effect: 0, supportedTargets: ActionTargets.Hostile, minLevel: 90)
            .AddOption(PrimalRendStrategy.Force, "Force use ASAP", cooldown: 0, effect: 0, supportedTargets: ActionTargets.Hostile, minLevel: 90)
            .AddOption(PrimalRendStrategy.GapClose, "Use Rend if outside melee range as a gap-closer", cooldown: 0, effect: 0, supportedTargets: ActionTargets.Hostile, minLevel: 90)
            .AddOption(PrimalRendStrategy.SmuggleNextPotion, "Delay until last possible GCD, if potion will be ready by the time ruin expires (useful for smuggling ruin into unaligned pot window)", cooldown: 0, effect: 0, supportedTargets: ActionTargets.Hostile, minLevel: 90)
            .AddOption(PrimalRendStrategy.SmuggleAlignedPotion, "Delay until last possible GCD, if potion will be ready by the time ruin expires and would overlap raidbuffs (useful for smuggling ruin into aligned pot window)", cooldown: 0, effect: 0, supportedTargets: ActionTargets.Hostile, minLevel: 90)
            .AddOption(PrimalRendStrategy.DelayUntilLastChance, "Delay until last possible GCD", cooldown: 0, effect: 0, supportedTargets: ActionTargets.Hostile, minLevel: 90)
