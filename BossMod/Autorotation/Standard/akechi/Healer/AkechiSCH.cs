...
public sealed class AkechiSCH(RotationModuleManager manager, Actor player) : AkechiTools<AID, TraitID>(manager, player)
{
    ...
    public static RotationModuleDefinition Definition()
    {
        ...
        res.Define(Track.Bio).As<BioStrategy>("Damage Over Time", "Bio", uiPriority: 190)
            .AddOption(BioStrategy.Bio3, "Use Bio if target has 3s or less remaining on DoT effect", cooldown: 0, effect: 30, supportedTargets: ActionTargets.Hostile, minLevel: 2)
            .AddOption(BioStrategy.Bio6, "Use Bio if target has 6s or less remaining on DoT effect", cooldown: 0, effect: 30, supportedTargets: ActionTargets.Hostile, minLevel: 2)
            .AddOption(BioStrategy.Bio9, "Use Bio if target has 9s or less remaining on DoT effect", cooldown: 0, effect: 30, supportedTargets: ActionTargets.Hostile, minLevel: 2)
            .AddOption(BioStrategy.Bio0, "Use Bio if target does not have DoT effect", cooldown: 0, effect: 30, supportedTargets: ActionTargets.Hostile, minLevel: 2)
            .AddOption(BioStrategy.Force, "Force use of Bio regardless of DoT effect", cooldown: 0, effect: 30, supportedTargets: ActionTargets.Hostile, minLevel: 2)
            .AddOption(BioStrategy.Delay, "Delay the use of Bio for manual or strategic usage", cooldown: 0, effect: 0, supportedTargets: ActionTargets.Hostile, minLevel: 2)
        ...
        res.Define(Track.EnergyDrain).As<EnergyStrategy>("Energy Drain", "E.Drain", uiPriority: 150)
            .AddOption(EnergyStrategy.Use3, "Uses all stacks of Aetherflow for Energy Drain; conserves no stacks for manual usage", cooldown: 0, effect: 0, supportedTargets: ActionTargets.Hostile, minLevel: 45)
            .AddOption(EnergyStrategy.Use2, "Uses 2 stacks of Aetherflow for Energy Drain; conserves 1 stack for manual usage, but consumes any remaining when Aetherflow ability is about to be up", cooldown: 0, effect: 0, supportedTargets: ActionTargets.Hostile, minLevel: 45)
            .AddOption(EnergyStrategy.Use1, "Uses 1 stack of Aetherflow for Energy Drain; conserves 2 stacks for manual usage, but consumes any remaining when Aetherflow ability is about to be up", cooldown: 0, effect: 0, supportedTargets: ActionTargets.Hostile, minLevel: 45)
            .AddOption(EnergyStrategy.Force, "Force use of Energy Drain if any Aetherflow is available", cooldown: 0, effect: 0, supportedTargets: ActionTargets.None, minLevel: 45)
            .AddOption(EnergyStrategy.Delay, "Delay use of Energy Drain", cooldown: 0, effect: 0, supportedTargets: ActionTargets.None, minLevel: 45)
